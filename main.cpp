...
// full game - PRESCORE/WRONG DEFEAT SCREEN
/*
#include <iostream>
#include <SDL.h>
#include <SDL_image.h>
...
int main(int game, char* games[])
{
    ...
    // Creating enemy bullet texture image
    SDL_Surface* bulletPic = IMG_Load("BloodBullet.png");
    SDL_SetColorKey(bulletPic, SDL_TRUE, SDL_MapRGB(bulletPic->format, 0xFF, 0xFF, 0xFF));
    SDL_Texture* bulletSpace = SDL_CreateTextureFromSurface(galagaField, bulletPic);
    SDL_FreeSurface(bulletPic);

    // Creating and loading defeat texture image
    SDL_Surface* defeatPic = IMG_Load("defeat.png");
    SDL_Texture* defeatScape = SDL_CreateTextureFromSurface(galagaField, defeatPic);
    SDL_FreeSurface(defeatPic);

    // Loading victory texture image
    SDL_Surface* victoryPic = IMG_Load("victory.png");
    SDL_Texture* victoryScape = SDL_CreateTextureFromSurface(galagaField, victoryPic);
    SDL_FreeSurface(victoryPic);
    ...
    while (gameEnd == false)
    {
        ...
        else if (victory == false && defeat == false)
        {
            ...
            for (int i = 0; i < shotCount; i++)
            {
                shot[i].checkCollision(Borg, collide[i], numOfBorg, numOfDestroyed);
                if (isShot == true && collide[i] == false)
                {
                    shot[i].renderTorpedo(galagaField, torpedoSpace);
                    shot[i].Move();
            ...
            for (int b = 0; b < darkShotCount; b++)
            {
                if (darkShot[b].active == true)
                {
                    darkShot[b].detectPlayerCollision(MillenniumFalcon, defeat);
                    darkShot[b].renderBullet(galagaField, bulletSpace);
                    darkShot[b].moveBullet();
                }
                if (darkShot[b].getBulletY() > 500)
...
// All enemies, all levels - PRE ENEMY FIRE
/*#include <iostream>
#include <SDL.h>
#include <SDL_image.h>
#include "PlayerShip.h"
...
// Move, Shoot, and Destroy Code - SINGLE ENEMY
/*
#include <iostream>
#include <SDL.h>
#include <SDL_image.h>
